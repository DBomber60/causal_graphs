...
Therefore, we can estimate $E(Y^{a'})$ by taking a weighted mean of the outcomes observed at treatment $a'$ where each unit is weighted by $1/P(A=a'|L)$. To illustrate a more general estimating framework, let us again consider the scenario where we have a binary treatment and let us refer to $E(Y^{1})$ as $\mu^1$. Further, we assume that we have $n$ experimental units/ subjects and we aim to estimate the causal risk difference, $\mu^1 - \mu^0$. To estimate the first quantity, we can solve the following estimating equation,
...
\end{align}
$$
where the sum is taken over the treated units, $t=\{i : a_i=1 \}$. This procedure (inverse probability weighting) can be interpreted/ conceptualized as generating a pseudopopulation in which treatment assignments do not depend on $L$. Therefore, in this reweighted sample, we have marginal exchangeability and can estimate the casual risk difference as the associational risk difference among this pseudopopulation. That is $\theta= E(\tilde{Y} \vert A=1) - E(\tilde{Y} \vert A=0)$.

#### Pseudopopulation Concept

One way to conceptualize the weights generated by IPW is a pseudopopulation in which all sample units undergo each of the treatment levels of interest. Within the $L=0$ segment, there are $8$ units, $4$ of whom were treated. Of the treated $1/4$ died. How many would have died if all of the units with $L=0$ had been treated? By conditional exchangeability, we would expect the same rate of death among the untreated, which would imply one more death had all of the units undergone treatment ($1/4 \times 4$). 
...
In the case where there are many treatment levels or treatment is continuous, it makes practical sense to hypothesize a functional realtionship between the treatment and response. For example, a linear relationship can be expressed,
...
which can accommodate a range of functions as well as treatment models (weights).
