...
And one is missing: $Y^{mis}=Y_i^{1-A_i}$. Bayesian inference considers the observed values of these quantities to be realizations of random variables and the unobserved values to be unobserved random variables, just like other model parameters. We assume that the joint distribution of the **complete** data (i.e., all counterfactual outcomes) depends on some parameters, $\Theta$, and we can express the joint distribution as,
...
1. $(Y^1, Y^0) \perp A$ (marginal exchangeability)
...
Bayesian causal inference typically proceeds by iteratively sampling $(Y^{miss}, \theta^Y)$ from $P(Y^{miss} \vert Y^{obs},A,\theta_Y)$ and $P(\theta^Y \vert Y^{miss}, Y^{obs}, A)$.
